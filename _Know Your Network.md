...
conf t
 int vlan 1
  ip add 10.71.1.2 255.255.255.0
...
Task:
 1. CoreTAAS must have the following SVIs
   - VLAN 1
       - IP address: 10.71.1.2
       - Description: DEFAULT-VLAN
       - Status: UP

   - VLAN 10
     - IP address: 10.71.10.2
     - Description: WIFI-VLAN
     - Status: UP

   - VLAN 50
     - IP address: 10.71.50.2
     - Description: CCTV-VLAN
     - Status: UP

   - VLAN 100
     - IP address: 10.71.100.2
...
conf t
 int vlan 1
  ...
  exit
 int vlan 10
  ip add 10.71.10.2 255.255.255.0
  description WIFI-VLAN
  no shut
  exit
 int vlan 50
  ip add 10.71.50.2 255.255.255.0
  description CCTV-VLAN
  no shut
  exit
 int vlan 100
  ip add 10.71.100.2 255.255.255.0
  description VOICE-VLAN
  no shut
 end
~~~
...
!@CoreBABA
conf t
 ip dhcp excluded-address 10.71.1.1 10.71.1.100
 ip dhcp excluded-address 10.71.10.1 10.71.10.100
 ip dhcp excluded-address 10.71.50.1 10.71.50.100
 ...
 ip dhcp pool POOLWIFI
  ...
  default-router 10.71.10.4
  domain-name WIFIDATA.COM
  dns-server 10.71.1.10
  option 43 ip 10.71.10.71
...
!@CoreBABA
conf t
 ip dhcp excluded-address 10.71.100.1 10.71.100.100
...
conf t
 ...
 ip dhcp excluded-add 10.71.10.1 10.71.10.100
...
conf t
 ...
 dial-peer voice 3 pots
  destination-pattern 7102
  port 0/0/2
 dial-peer voice 4 pots
  destination-pattern 7103
  port 0/0/3
 end
~~~
...
conf t
 ...
 ip route 10.71.0.0 255.255.0.0 10.71.71.4 254
 end
~~~
...
conf t
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.71.100.4 254
 end
~~~
...
!@CoreBABA
conf t
 ip route 0.0.0.0 0.0.0.0 10.71.71.1 254
 end
~~~
...
~~~
!@cmd
route add 10.0.0.0 mask 255.0.0.0 10.71.1.4
route add 200.0.0.0 mask 255.255.255.0 10.71.1.4
~~~
...
~~~
!@cmd
ping 10.71.1.2                 CoreTAAS
ping 10.71.1.4                 CoreBABA
ping 10.71.100.8               CUCM
ping 10.71.71.1            EDGE
~~~
